...
export class RegisterOpsinService {
    ...
    create(giat: RegisterOpsin) {
        return this.httpClient.post<RegisterOpsin>(this.endPoint, giat)
    ...
    update(giat: RegisterOpsin) {
        const putEndPoint = `${this.endPoint}/${giat.id}`;
        return this.httpClient.put<RegisterOpsin>(putEndPoint, giat)
            .pipe(catchError(errorResponse => {
                let errorMessage = 'Aduh!!!... Gawat nih bro... GAGAL terhubung ke server';
                if (!errorResponse.error) {
                    return throwError(() => errorMessage);
